...
export const SCHEME = "literate-comments";
...
class WatchedMap {
	...
	remove(chunk: Chunk) {
		...
		if (activeCount === 0) this.activeWatches.delete(key);
		else this.activeWatches.set(key, activeCount);

		if (chunks.length === 0) this.documentChunks.delete(documentKey);
	}
...
class ChunkCache {
	...
	async refresh(chunk: Chunk) {
		const key = chunk.key();
		if (this.map.has(key)) this.map.set(key, await this.generate(chunk));
	}
...
export class MarkdownFileProvider implements FileSystemProvider, Disposable {
	...
	async readFile(uri: Uri): Promise<Uint8Array> {
		...
		if (this.active.has(chunk.documentKey())) return new TextEncoder().encode(await this.cache.text(chunk));
		else {
			try {
	...
	async stat(uri: Uri): Promise<FileStat> {
		const chunk = Chunk.fromUri(uri);
		if (this.active.has(chunk.documentKey())) return await this.cache.stat(Chunk.fromUri(uri));
		else {
			try {
	...
	watch(uri: Uri, options: { readonly recursive: boolean; readonly excludes: readonly string[] }): Disposable {
		...
		return {
			dispose() {
				...
				if (!self.watchedMap.has(chunk)) self.cache.forget(chunk);
			},
		};
	}
