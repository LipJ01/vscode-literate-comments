...
export class DocumentMap {
	...
	textInRange(start: Position, end: Position, lineSeparator: string = "\n") {
		...
		if (start.line === end.line) return this.text(start.line).substring(start.character, end.character + 1);

		let text = this.text(start.line).substring(start.character);
		text += lineSeparator;
		for (let line = start.line + 1; line < end.line; line++) {
			text += this.text(line);
			text += lineSeparator;
		}
		text += this.text(end.line).substring(0, end.character + 1);
		return text;
	}
	...
	private nextComment(after?: Position): CommentGroup | undefined {
		...
		const blockStart = !hasBlock
			? undefined
			: this.positionOf((line, character) => line.indexOf(this.commentSyntax.blockStart!, character), after);
		const blockFirst = blockStart !== undefined && (lineStart === undefined || lineStart.isAfter(blockStart));
		if (blockFirst) {
			const blockEnd = this.positionOf(
				(line, character) => line.indexOf(this.commentSyntax.blockEnd!, character),
				blockStart
			);
			return {
				block: true,
